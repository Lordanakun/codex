...
impl App {
    ...
    pub async fn run(
        ...
        let exit_reason = loop {
            ...
            // Some replayed slash commands pause queue draining until their app-side updates or
            // popup flows settle. Only resume once the app-event queue is fully drained so multi-
            // event commands (for example approvals updates) cannot interleave later queued input.
